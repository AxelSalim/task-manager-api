...
export function EditHabitSheet({ open, onOpenChange, habit, onSaved }: EditHabitSheetProps) {
  const [habitName, setHabitName] = useState('');
  const [saving, setSaving] = useState(false);
  const { toast } = useToast();

  useEffect(() => {
    if (open && habit) setHabitName(habit.name);
  }, [open, habit?.name]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetHeader className="shrink-0 border-b px-5 py-3">
          <SheetTitle className="text-lg">Modifier l&apos;habitude</SheetTitle>
        </SheetHeader>
